...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function autoScroll() {
    currentTranslate -= 1; // Скорость прокрутки
    if (Math.abs(currentTranslate) >= sliderTrack.scrollWidth / 2) {
      currentTranslate = 0; // Сброс до начала для бесшовного эффекта
    }
  ...
  sliderTrack.addEventListener("mousedown", (e) => {
    ...
    sliderTrack.style.cursor = "grabbing"; // Меняем курсор на 'захват'
    cancelAnimationFrame(animationID); // Останавливаем автопрокрутку
  });
  ...
  sliderTrack.addEventListener("mouseup", () => {
    isDragging = false;
    sliderTrack.style.cursor = "grab"; // Возвращаем курсор
  ...
  // Взаимодействие с тач-скрином (для мобильных)
